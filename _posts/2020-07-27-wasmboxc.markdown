...
when changing the compiler, for example the "none" sandboxing results go from
...
And here's how easy it is to use WasmBoxC to get a fully sandboxed library linked with our normal trusted code:
...
wasm toolchain components like clang and Emscripten, and standard C compilers. The key wasm2c
